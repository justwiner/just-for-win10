...
.calendar-years-content::-webkit-scrollbar {
  display: none;
}
.calendar-years-bg {
...
.calendar-years-content>span.active{
    border-color: #0078D7;
    background-color: #0078D7;
    color:#fff; 
}
.calendar-years-content>span:hover {
    border-color: #7D7D7F;
}
</style>
...
<template>
    <div class="calendar">
                ...
                <span class="calendar-prev" @click="prev">
                    ...
                    <g class="transform-group">
                        <g transform="scale(0.015625, 0.015625)">
                            <path d="M671.968 912c-12.288 0-24.576-4.672-33.952-14.048L286.048 545.984c-18.752-18.72-18.752-49.12 0-67.872l351.968-352c18.752-18.752 49.12-18.752 67.872 0 18.752 18.72 18.752 49.12 0 67.872l-318.016 318.048 318.016 318.016c18.752 18.752 18.752 49.12 0 67.872C696.544 907.328 684.256 912 671.968 912z" fill="#CCCCCC"></path>
                        </g>
                    </g>
                    </svg>
                </span>
        ...
        <div class="calendar-years" :class="{'show':yearsShow}">
            <div class="calendar-years-bg"></div>
                ...
                if (this.range) { // 范围
                    ...
                    if (parseInt(seletSplit[0]) == this.year && parseInt(seletSplit[1]) - 1 == this.month && parseInt(seletSplit[2]) == i) {
                     ...
                     // 没有默认值的时候显示选中今天日期
                        ...
                        if (this.begin.length>0) {
                            let beginTime = Number(new Date(parseInt(this.begin[0]),parseInt(this.begin[1]) - 1,parseInt(this.begin[2])))
                            if (beginTime > Number(new Date(this.year, this.month, i))) assItem.disabled = true
                        }
                        if (this.end.length>0){
                            let endTime = Number(new Date(parseInt(this.end[0]),parseInt(this.end[1]) - 1,parseInt(this.end[2])))
                            if (endTime <  Number(new Date(this.year, this.month, i))) assItem.disabled = true
                        }
                        if (this.disabled.length>0){
                            if (this.disabled.filter(v => {return this.year === v[0] && this.month === v[1]-1 && i === v[2] }).length>0) {
                                assItem.disabled = true
                            }
                        }
                    }
                    if (this.year === chkY && this.month === chkM && i === chkD) {
                        assItem.isToday = true
                    }
                    temp[line].push(assItem)
                }
                // 到周六换行
